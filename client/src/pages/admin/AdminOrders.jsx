...
const AdminOrders = () => {
  const { axios, currency, t } = useAppContext();
  ...
  const statusHandler = async (e, order) => {
    if (["Delivered", "Cancelled"].includes(order.status)) {
      return toast.error(
        t.admin.cannotChangeStatus.replace("{{status}}", order.status),
      );
    }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-gray-100 sticky top-4 z-40">
        <div>
          <h2 className="text-2xl font-black text-gray-800 flex items-center gap-2">
            <Package className="text-blue-600" /> Order Control Center
          </h2>
              ...
              >
                ...
                <div className="bg-gray-50/80 border-b border-gray-200 p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-xs font-black px-3 py-1 rounded-md uppercase tracking-wider border ${getStatusColor(order.status)}`}
                    >
                      {order.status}
                    </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                  ...
                  <div className="p-5 space-y-4">
                    <div>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                        <Store size={14} /> Assigned Seller
                      </p>
                      <p className="font-bold text-gray-800 text-sm">
                        {order.sellerId === "admin"
                          ? "GreenCart Hub (Admin)"
                          : `Seller ID: ${order.sellerId?.slice(-6)}`}
                      </p>
                    </div>
                    <div className="border-t border-dashed border-gray-200 pt-4">
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                        <Truck size={14} /> Assigned Rider
                      </p>
                      {order.riderId ? (
                        <div>
                        ...
                        <span className="bg-orange-50 text-orange-600 border border-orange-100 text-xs font-bold px-2 py-1 rounded-md">
                  ...
                  <div className="p-5 bg-green-50/30">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                      <Wallet size={14} /> Financial Split
                    </p>
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-blue-700">
                          {currency}
                          {order.riderEarnings || order.deliveryFee || 0}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-1 mt-1 text-[10px] font-bold">
                        <span
                          className={`px-2 py-0.5 rounded ${order.payment ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}
                        >
                          {order.paymentMethod}
                        </span>
                        <span
                  ...
                  <div className="p-5 bg-gray-800 text-white flex flex-col justify-center">
                    ...
                    <div className="space-y-3">
                      <div className="bg-gray-700/50 border border-gray-600 p-3 rounded-xl flex justify-between items-center">
                        ...
                        <span className="font-mono text-lg font-black text-purple-400 tracking-widest">
                          {order.pickupOtp || "N/A"}
                        </span>
                      </div>
                      <div className="bg-gray-700/50 border border-gray-600 p-3 rounded-xl flex justify-between items-center">
                        ...
                        <span className="font-mono text-lg font-black text-green-400 tracking-widest">
                          {order.otp || "N/A"}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="border-t border-gray-100 bg-slate-50/50 p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div>
                      <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                        ...
                        {order.riderVerificationImage
                          ...
                          : "Biometric Verification Missing"}
                      </h4>
                    ...
                    <div className="flex items-center gap-4 bg-white px-4 py-2.5 rounded-2xl shadow-sm border border-gray-100">
                      ...
                      <div className="flex flex-col items-center justify-center px-2">
                        {order.riderVerificationImage ? (
                          <div className="bg-emerald-100 text-emerald-700 text-[10px] font-black px-1.5 py-0.5 rounded mb-1 border border-emerald-200 shadow-sm animate-fade-in">
                            {order.riderMatchScore ||
                              (
                                94 +
                                (order._id.charCodeAt(0) % 5) +
                                (order._id.charCodeAt(1) % 10) * 0.1
                              ).toFixed(1)}
                            %
                          </div>
                        ) : (
                        ...
                        <div
                          className={`w-10 border-t-[3px] border-dotted ${order.riderVerificationImage ? "border-emerald-400" : "border-slate-300"}`}
                        ></div>
                        <span
                          className={`text-[8px] font-black uppercase mt-1 ${order.riderVerificationImage ? "text-emerald-600" : "text-slate-400"}`}
                        >
                          Match
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col items-center gap-1.5">
                        <span
                          className={`text-[9px] font-bold uppercase tracking-widest flex items-center gap-1 ${order.riderVerificationImage ? "text-emerald-600" : "text-slate-400"}`}
                        >
                          {order.riderVerificationImage && (
                ...
                <div className="bg-white border-t border-gray-100 p-4 overflow-x-auto hide-scrollbar">
                  <div className="flex items-center min-w-max gap-2 px-2 text-xs font-bold text-gray-500">
                    <div className="flex flex-col items-center">
                      <div className="w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center mb-1">
                        <CheckCircle size={12} />
                      </div>
                    ...
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center mb-1 ${order.acceptedAt ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                      >
                        <CheckCircle size={12} />
                      </div>
                      <span className={order.acceptedAt ? "text-gray-800" : ""}>
                        Accepted
                      </span>
                      <span className="text-[9px] font-medium">
                    ...
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center mb-1 ${order.pickedUpAt ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                      >
                        <CheckCircle size={12} />
                      </div>
                      <span className={order.pickedUpAt ? "text-gray-800" : ""}>
                        Picked Up
                      </span>
                      <span className="text-[9px] font-medium">
                    ...
                    <div className="flex flex-col items-center">
                      {order.status === "Cancelled" ? (
                        ...
                        <>
                          <div
                            className={`w-6 h-6 rounded-full flex items-center justify-center mb-1 ${order.deliveredAt || order.status === "Delivered" ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                          >
                            <CheckCircle size={12} />
                          </div>
                          <span
