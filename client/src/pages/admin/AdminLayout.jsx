...
const AdminLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-outfit overflow-hidden">
      ...
      <div
        className={`fixed lg:relative top-0 left-0 w-[280px] h-full bg-white border-r border-slate-200 flex flex-col shadow-2xl lg:shadow-none z-50 transition-transform duration-300 ease-in-out lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {/* Sidebar Header */}
        <div className="p-6 border-b border-slate-100 flex flex-col gap-3 shrink-0">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => setSidebarOpen(false)}
              className="lg:hidden text-slate-400 hover:text-slate-800 bg-slate-50 p-1.5 rounded-lg border border-slate-200"
            >
              <X size={16} />
            </button>
          </div>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto custom-scrollbar">
          ...
          {sidebarLinks.map((item) => (
            <NavLink
              ...
              className={({
                isActive,
              }) => `group flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all font-bold text-sm
            ...
            >
              {({ isActive }) => (
                <>
                  <item.icon
                    size={18}
                    className={`transition-transform duration-300 ${isActive ? "scale-110 text-white" : "group-hover:scale-110 group-hover:text-emerald-500"}`}
                  />
                  {item.name}
                </>
              )}
        ...
        <div className="p-4 border-t border-slate-100 space-y-2 bg-slate-50 shrink-0">
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-3 rounded-xl text-slate-500 hover:bg-white border border-transparent hover:border-slate-200 hover:text-slate-800 transition-all shadow-sm text-sm font-bold group"
          >
          ...
          <button
            onClick={logout}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-rose-500 hover:bg-rose-50 hover:text-rose-600 transition-colors text-sm font-bold"
          >
