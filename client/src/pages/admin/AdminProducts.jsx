...
const AdminProducts = () => {
  const { axios, currency, token, t } = useAppContext();
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      toast.error(t.admin.errorLoadingProducts);
    } finally {
      setLoading(false);
    }
  };
  ...
  const approveProduct = async (id) => {
    const loadToast = toast.loading(t.admin.approvingProduct);
    try {
  ...
  const removeProduct = async (id) => {
    if (!window.confirm(t.admin.deleteProductPermanently)) return;
    try {
      const { data } = await axios.post(
        "/api/product/remove",
        { id },
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="space-y-6 pb-20 font-outfit"
    >
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        ...
        <Link
          ...
          className="bg-slate-900 hover:bg-slate-800 text-white px-5 py-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-xl shadow-slate-200 transition-all active:scale-95 w-full md:w-auto"
        >
      ...
      <div className="flex bg-white p-1.5 rounded-xl shadow-sm border border-slate-200 w-max">
        ...
        >
          <CheckCircle
            size={16}
            className={activeTab === "live" ? "text-emerald-400" : ""}
          />
        ...
        >
          <Clock
            size={16}
            className={activeTab === "pending" ? "text-orange-400" : ""}
          />
          Pending Approval
                  ...
                  <td colSpan="5" className="p-16 text-center">
                    <div className="flex flex-col items-center justify-center text-slate-400">
                      {activeTab === "pending" ? (
                        <AlertCircle
                          size={40}
                          className="mb-4 opacity-50 text-orange-400"
                        />
                      ) : (
                      ...
                      <div className="flex items-center justify-center gap-3">
                        ...
                        {activeTab === "pending" && (
                          <button
                            ...
                            className="bg-emerald-50 border border-emerald-200 text-emerald-600 px-4 py-2 rounded-xl text-sm font-bold hover:bg-emerald-100 transition-colors shadow-sm flex items-center gap-1.5"
                        ...
                        <button
                          ...
                          className="p-2.5 text-indigo-500 hover:bg-indigo-50 rounded-xl transition-all hover:scale-110"
                          title="Edit"
                        >
                          <Edit size={18} />
                        </button>
                        ...
                        <button
                          ...
                          className="p-2.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all hover:scale-110"
                          title={
                            activeTab === "pending"
                              ? "Reject & Delete"
                              : "Delete"
