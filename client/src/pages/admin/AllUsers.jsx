...
const AllUsers = () => {
  const { axios, currency, role: currentUserRole, t } = useAppContext();
  ...
  const fetchDeepInfo = async (user) => {
    ...
    try {
      const { data } = await axios.post("/api/user/admin/deep-view", {
  ...
  const handlePromote = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          `User successfully promoted to ${newRole.toUpperCase()}!`,
          { id: loadToast },
        );
  ...
  const toggleBan = async (id) => {
    ...
    try {
      const { data } = await axios.post("/api/user/admin/block", {
        userId: id,
      });
      if (data.success) {
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        "Delete user permanently? This removes them from Auth completely.",
      )
    )
      return;
    try {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="space-y-6 relative font-outfit pb-20"
    >
                    ...
                    <td className="p-5">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-2 text-indigo-700 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 rounded-lg transition-colors font-bold text-xs flex items-center gap-1.5 shadow-sm"
                          title="Promote User"
                        >
                        ...
                        <button
                          ...
                          className="p-2 text-slate-500 bg-white border border-slate-200 hover:text-slate-800 hover:bg-slate-50 rounded-lg transition-colors shadow-sm"
                          title="Edit Info"
                        >
                          <Edit size={16} />
                        </button>
                        ...
                        <button
                          ...
                          className={`p-2 border rounded-lg transition-colors shadow-sm ${user.isBlocked ? "text-emerald-600 bg-emerald-50 border-emerald-200 hover:bg-emerald-100" : "text-amber-600 bg-amber-50 border-amber-200 hover:bg-amber-100"}`}
        ...
        {showDeepView && deepData && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 md:p-8">
            <motion.div
              ...
              className="bg-white rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col border border-slate-200"
            >
              <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50 shrink-0">
                <div>
                  <h3 className="font-bold text-lg text-slate-900 flex items-center gap-2">
                    <Activity size={18} className="text-blue-600" /> Customer
                    Analytics Profile
                  </h3>
                  <p className="text-[11px] text-slate-500 font-bold mt-1 uppercase tracking-widest">
                    {deepData.user.name} | {deepData.user.email}
                  </p>
                </div>
                <button
                  onClick={() => setShowDeepView(false)}
                  className="p-2 bg-white rounded-md text-slate-400 hover:text-slate-800 shadow-sm border border-slate-200 transition-colors"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto space-y-6 bg-slate-50/50 flex-1 hide-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                      ...
                      Recent Value (Last 10)
                    </p>
                    <p className="text-2xl font-bold text-slate-900">
                      {currency}
                      {calculateRecentSpend(deepData.orders).toLocaleString()}
                    </p>
                    <p className="text-[11px] text-slate-400 mt-1 font-medium">
                      {deepData.orders.length} total recorded orders
                    </p>
                  </div>
                  <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                      <Package size={14} className="text-indigo-500" /> Live
                      Cart State
                    </p>
                    <p className="text-2xl font-bold text-slate-900">
                      {Object.keys(deepData.cart).length}
                    </p>
                    <p className="text-[11px] text-slate-400 mt-1 font-medium">
                      Unique items in active cart
                    </p>
                  </div>
                  <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                      <Calendar size={14} className="text-blue-500" /> Account
                      Age
                    </p>
                    <p className="text-2xl font-bold text-slate-900">
                      {new Date(deepData.user.createdAt).toLocaleDateString()}
                    </p>
                    <p className="text-[11px] text-slate-400 mt-1 font-medium">
                      Joined platform
                    </p>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                      <div>
                        <p className="text-base font-bold text-slate-900">
                          Order #
                          {deepData.orders[0]._id.slice(-8).toUpperCase()}
                        </p>
                        <p className="text-xs text-slate-600 mt-1">
                          <span className="font-semibold text-slate-800">
                            Placed:
                          </span>{" "}
                          {new Date(deepData.orders[0].date).toLocaleString()}
                        </p>
                      ...
                      <div className="flex flex-col items-end gap-2 bg-white p-3.5 rounded-lg border border-blue-50 shadow-sm w-full md:w-auto">
                        <p className="text-lg font-black text-slate-900">
                          {currency}
                          {deepData.orders[0].amount}
                        </p>
                        <div className="flex gap-2">
                          <span
                            className={`px-2 py-1 text-[9px] font-bold uppercase tracking-wider rounded border ${deepData.orders[0].status === "Delivered" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
                          >
                            {deepData.orders[0].status}
                          </span>
                          <span
                            className={`flex items-center gap-1 px-2 py-1 text-[9px] font-bold uppercase tracking-wider rounded border ${deepData.orders[0].paymentMethod === "COD" ? "bg-slate-100 text-slate-700 border-slate-300" : "bg-indigo-50 text-indigo-700 border-indigo-200"}`}
                          >
                            {deepData.orders[0].paymentMethod === "COD" ? (
                              <Banknote size={10} />
                            ) : (
                              <CreditCard size={10} />
                            )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-1 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col max-h-[300px]">
                    <div className="p-4 border-b border-slate-100 bg-slate-50">
                      <h3 className="font-bold text-slate-800 text-xs flex items-center gap-2">
                        <MapPin size={14} className="text-rose-500" /> Saved
                        Logistics Data
                      </h3>
                    </div>
                  ...
                  <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col max-h-[300px]">
                    <div className="p-4 border-b border-slate-100 bg-slate-50">
                      <h3 className="font-bold text-slate-800 text-xs flex items-center gap-2">
                        <Clock size={14} className="text-emerald-500" /> Order
                        Ledger (Last 10)
                      </h3>
                    </div>
                    <div className="overflow-y-auto hide-scrollbar flex-1">
                      <table className="w-full text-left text-sm">
                        <thead className="bg-white sticky top-0 border-b border-slate-100 shadow-sm">
                          <tr>
                            <th className="p-3 text-[9px] text-slate-500 uppercase tracking-widest font-bold">
                              Order ID / Date
                            </th>
                            <th className="p-3 text-[9px] text-slate-500 uppercase tracking-widest font-bold">
                              Items
                            </th>
                            <th className="p-3 text-[9px] text-slate-500 uppercase tracking-widest font-bold text-right">
                              Value / Status
                            </th>
                          </tr>
                        </thead>
                            ...
                            deepData.orders.map((o, i) => (
                              ...
                              >
                                ...
                                <td className="p-3 text-right">
                                  ...
                                  <p
                                    className={`text-[9px] font-bold uppercase tracking-wider mt-0.5 ${o.status === "Delivered" ? "text-emerald-600" : o.status === "Cancelled" ? "text-rose-500" : "text-amber-600"}`}
                                  >
                                    {o.status}
                                  </p>
                                </td>
            ...
            >
              <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                <h3 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                  <Edit size={16} className="text-slate-500" /> Edit Details
                </h3>
                <button
              ...
              <form onSubmit={handleUpdate} className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    required
                    className="w-full border border-slate-300 p-2.5 rounded-lg mt-1.5 text-sm font-medium text-slate-800 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-shadow"
                    value={editData.name}
                    onChange={(e) =>
        ...
        {showPromote && selectedUser && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.95, opacity: 0, y: 10 }}
              className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl border-t-4 border-indigo-500"
            >
              <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                <h3 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                  <ArrowUpCircle size={16} className="text-indigo-600" />{" "}
                  Promote User
                </h3>
                <button
              ...
              <form onSubmit={handlePromote} className="p-6 space-y-5">
                <p className="text-sm text-slate-600">
                  ...
                  <b className="text-slate-900">{selectedUser.name}</b>
                </p>
