...
const SuperNotifications = () => {
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    if (!title || !message)
      return toast.error("Please fill all announcement fields");
    if (target === "specific" && !targetEmail)
      return toast.error("Please enter the user's email");
    ...
    try {
      ...
      const payload = {
        ...
        message,
        target,
        email: target === "specific" ? targetEmail : null,
      };
    ...
    <div className="max-w-3xl mx-auto space-y-8 pb-20">
      <div>
        ...
        <p className="text-slate-500 mt-2 font-medium">
          Deploy instant push notifications to user dashboards.
        </p>
      </div>

      <form
        onSubmit={handleSend}
        className="bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-200 space-y-8 relative overflow-hidden"
      >
        {/* Decorative Blur */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              >
                <role.icon
                  size={20}
                  className={target === role.id ? "animate-bounce" : ""}
                />
        ...
        {target === "specific" && (
          ...
          >
            ...
            <input
              ...
              placeholder="ex: user@kuakumarket.com"
        ...
        <button
          disabled={loading}
          className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 flex items-center justify-center gap-2 active:scale-95 disabled:opacity-70 relative z-10"
        >
          {loading ? (
            "Transmitting..."
          ) : (
