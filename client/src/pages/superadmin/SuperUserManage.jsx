...
const SuperUserManage = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      toast.error(t.superadmin.failedSyncUserLedger);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBan = async (id, isBlocked) => {
    if (
      !window.confirm(
        isBlocked ? t.superadmin.areSureRestore : t.superadmin.areSureSuspend,
      )
    )
      return;
    try {
      ...
      if (data.success) {
        ...
        toast.error(t.superadmin.actionFailed);!window.confirm(t.superadmin.criticalWarning)
      return;
    try {
  ...
  const handlePasswordReset = async (e) => {
    e.preventDefault();
    if (newPass.length < 8)
      return toast.error(t.superadmin.passwordMustBe8Chars);

    const loadToast = toast.loading(t.superadmin.updatingSecurityCredentials);
    try {
      const { data } = await axios.post("/api/user/admin/reset-password", {
        userId: selectedUser._id,
        newPassword: newPass,
      });
      if (data.success) {
        toast.success(t.superadmin.securityCredentialsUpdated, { id: loadToast });
  ...
  const handleRoleChange = async (e) => {
    ...
    if (newRole === selectedUser.role)
      return toast.error(t.superadmin.userAlreadyHasRole);

    const loadToast = toast.loading(
      t.superadmin.updateAccessLevelTo.replace('{{role}}', newRole.toUpperCase()),
    );
    try {
      const payload = {
        userId: selectedUser._id,
    ...
    <div className="space-y-6 pb-12 w-full min-w-0">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-5 lg:p-6 rounded-2xl shadow-sm border border-slate-200/60 w-full">
        <div>
          <h1 className="text-xl lg:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <Users size={24} className="text-indigo-600" /> Network User
            Management
          </h1>
          <p className="text-xs text-slate-500 mt-1 font-bold uppercase tracking-widest">
            Review, promote, and enforce security policies
          </p>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex items-start sm:items-center gap-3 text-blue-800 text-xs font-medium shadow-sm w-full">
        ...
        <p className="leading-relaxed">
          <strong>Clerk Auth Sync Active:</strong> New users must register via
          the public portal. Use the{" "}
          <b className="text-blue-900 font-bold">Modify Access Level</b> action
          to promote existing users to Sellers, Riders, or Admins.
        </p>
      </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td
                    ...
                    className="p-10 text-center text-slate-400 font-bold uppercase tracking-widest text-xs"
                  >
                    No {roleTab} records found matching your criteria.
                  </td>
                </tr>
              ) : (
                filtered.map((u) => (
                  ...
                  >
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full border flex items-center justify-center font-black text-sm shrink-0
                                                ${
                                                  roleTab === "seller"
                    ...
                    <td className="p-5 align-middle">
                      ...
                      {u.role === "user" && (
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest px-2.5 py-1 bg-slate-50 border border-slate-100 rounded-md">
                          Standard Consumer
                        </span>
                      )}
                    </td>
                    ...
                    <td className="p-5 align-middle">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-[10px] font-black uppercase tracking-wider ${u.isBlocked ? "bg-rose-50 text-rose-600 border-rose-200" : "bg-emerald-50 text-emerald-600 border-emerald-200"}`}
                      >
                        {u.isBlocked ? (
                    ...
                    <td className="p-5 align-middle">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[9px] text-slate-400 flex items-center gap-1 mt-1 font-bold uppercase tracking-wider">
                    ...
                    <td className="p-5 text-right align-middle">
                      ...
                      <div className="flex items-center justify-center gap-2 transition-opacity">
                        ...
                        <button
                          ...
                          title={
                            u.isBlocked ? "Restore Access" : "Suspend Access"
                          }
                        >
                          <ShieldBan size={16} />
                        </button>
              ...
              <div className="bg-slate-50 p-5 border-b border-slate-200 flex justify-between items-center">
                <div>
                  <h3 className="font-bold text-sm text-slate-900 flex items-center gap-2">
                    <UserCog size={16} className="text-blue-600" /> Modify
                    Access Level
                  </h3>
                ...
                <div>
                  ...
                  <div className="space-y-2.5">
                    {["user", "seller", "rider", "admin"].map((r) => (
                      <label
                        key={r}
                        className={`flex items-center gap-3 p-3.5 border rounded-xl cursor-pointer transition-all shadow-sm ${newRole === r ? "border-blue-500 bg-blue-50 ring-1 ring-blue-500" : "border-slate-200 bg-white hover:bg-slate-50"}`}
                      >
                        <input
                          type="radio"
                          name="roleGroup"
                        ...
                        />
                        <span
                          className={`text-sm font-bold capitalize ${newRole === r ? "text-blue-900" : "text-slate-700"}`}
                        >
                          {r}
                        </span>
                      </label>
                  ...
                  {newRole === "admin" && (
                    <p className="text-[10px] font-bold text-rose-500 mt-4 flex items-center gap-1.5 bg-rose-50 p-2.5 rounded-lg border border-rose-100">
                      <AlertTriangle size={14} className="shrink-0" /> Warning:
                      Grants administrative system access.
                    </p>
                  )}
                </div>
                <button
                  type="submit"
                  className="w-full bg-blue-600 text-white py-3 rounded-xl text-sm font-bold hover:bg-blue-700 transition-all active:scale-95 shadow-md flex items-center justify-center gap-2"
                >
                  Authorize Role Transfer
                </button>
              </form>
            </motion.div>
          </div>
              ...
              <div className="bg-amber-50 p-5 border-b border-amber-200 flex justify-between items-center">
                <div>
                  <h3 className="font-bold text-sm text-amber-900 flex items-center gap-2">
                    <Key size={16} className="text-amber-600" /> Security
                    Override
                  </h3>
                  <p className="text-[10px] text-amber-600 uppercase tracking-widest mt-1 font-bold">
                    Target: {selectedUser?.email}
                  </p>
                </div>
                <button
              ...
              <form onSubmit={handlePasswordReset} className="p-6 space-y-5">
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-2 font-medium leading-relaxed">
                    This will instantly overwrite the user's password in the
                    central Clerk Authentication database and terminate active
                    sessions.
                  </p>
                </div>
                <button
                  type="submit"
                  className="w-full bg-amber-500 text-white py-3 rounded-xl text-sm font-bold hover:bg-amber-600 transition-all active:scale-95 shadow-md flex items-center justify-center gap-2"
                >
                  <Shield size={16} /> Force Credentials Reset
                </button>
              </form>
            </motion.div>
          </div>
