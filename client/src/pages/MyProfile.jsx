...
const DisplayField = ({
  ...
  <div className="space-y-1.5">
    ...
    {isEditing ? (
      ...
      <div className="text-sm font-semibold text-gray-900 pb-2">
        {value || (
...
const MyProfile = () => {
  const { user, setUser, axios: api, isLoading, t } = useContext(AppContext);
  ...
  const [userData, setUserData] = useState({
    ...
    vehicleNumber: "",
    upiId: "",
    bankAccount: { bankName: "", accountNumber: "", ifsc: "" },
  ...
  useEffect(() => {
    if (user) {
      ...
      if (
        fetchedBankName &&
        !INDIAN_BANKS.includes(fetchedBankName) &&
        fetchedBankName !== "Other (Add manually)"
      ) {
  ...
  const handleNestedChange = (e, parent) =>
    setUserData((prev) => ({
      ...prev,
      [parent]: { ...prev[parent], [e.target.name]: e.target.value },
    }));
  ...
  const handleImageSelect = (e) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024)
        return toast.error(t.profileSettings.imageMustBeUnder);
  ...
  const handleApplyCrop = async () => {
    try {
      ...
      const croppedFile = new File([croppedBlob], "profile_picture.jpg", {
        type: "image/jpeg",
      });
      ...
      console.error(e);
      toast.error(t.profileSettings.failedCropImage);
    }
  };

  const handleCurrentLocation = () => {
    if (!navigator.geolocation)
      return toast.error(t.profileSettings.browserBlocksGeolocation);
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          if (data?.address) {
            ...
            toast.success(t.profileSettings.locationAutoFilledSuccessfully);
            setShowMapModal(false);
          }
        } catch (err) {
          toast.error(t.profileSettings.addressLookupFailed);
        } finally {
      ...
      () => {
        setIsLoadingLocation(false);
        toast.error(t.profileSettings.locationAccessDenied);
      },
    );
  };
  ...
  const handleMapSelect = (locationDetails) => {
    ...
    setShowMapModal(false);
    toast.success(t.profileSettings.locationPinnedSuccessfully);
  };

  const handleUpdate = async () => {
    setIsSaving(true);
    const loadToast = toast.loading(t.profileSettings.syncingProfileSecurely);
    ...
    try {
      ...
      if (data.success) {
        toast.success(t.profileSettings.profileFullySynced, { id: loadToast });
        setUser(data.user);
        setIsEditing(false);
        setImageFile(null);
      } else {
      ...
      toast.error(t.profileSettings.failedSyncProfile, { id: loadToast });
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden mb-8">
          ...
          <div className="px-8 pb-8 relative">
            <div className="flex flex-col sm:flex-row sm:items-end gap-6 -mt-12 mb-6">
              <div
                className="relative group w-28 h-28 rounded-full border-4 border-white bg-white shadow-md flex-shrink-0 cursor-pointer"
              ...
              <div className="pb-1">
                ...
                <div
                  className={`inline-flex items-center gap-1.5 mt-1.5 px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider ${theme.bg} ${theme.color} border ${theme.border}`}
                >
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-8">
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
              <h3 className="text-sm font-bold text-gray-800 uppercase tracking-widest mb-6 border-b border-gray-100 pb-4">
                Personal Details
              </h3>
                ...
                <div className="grid sm:grid-cols-2 gap-x-8 gap-y-6">
                  ...
                  <div className="sm:col-span-2 bg-slate-50 p-6 rounded-2xl border border-slate-200 mt-2">
                    <div className="flex items-center gap-2 text-sm font-bold text-slate-800 mb-5">
                      <CreditCard size={18} className="text-slate-500" /> Direct
                      Bank Transfer
                    </div>
                    <div className="grid sm:grid-cols-2 gap-x-6 gap-y-5">
                      ...
                      <DisplayField
                        ...
                        name="ifsc"
                        parent="bankAccount"
                        value={userData.bankAccount.ifsc}
                        isEditing={isEditing}
                        onNestedChange={handleNestedChange}
            ...
            >
              <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-medium">
                    Adjust the image to fit your profile avatar.
                  </p>
                </div>
                <button
                  onClick={() => setShowCropper(false)}
                  className="p-2 bg-white border border-gray-200 hover:bg-gray-100 rounded-full transition-colors shadow-sm"
                >
              ...
              <div className="p-5 bg-white border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  <button
                    onClick={() => setShowCropper(false)}
                    className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl font-bold text-sm bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={handleApplyCrop}
                    className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl font-bold text-sm bg-indigo-600 text-white shadow-md hover:bg-indigo-700 transition-colors"
                  >
                    Apply Crop
                  </button>
                </div>
            ...
            >
              <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-medium">
                    Search for your shop, drag the pin, or auto-detect.
                  </p>
                </div>
